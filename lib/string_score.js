...
function rmsScore(str){
  ...
  score += (othersProb(uStr) - 0.02) * (othersProb(uStr) - 0.02);
  return score  
}

function othersProb(str){
	letters = 0;
	spaces = 0;
	var alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
	for (var i=0;i<str.length;i++){
		if (alphabet.indexOf(str.toUpperCase()) != -1){
			letters++;
		} else if (str.toUpperCase()[i] == " "){
			spaces++;
		}
	}
	return (str.length - spaces - letters)/str.length;
}
